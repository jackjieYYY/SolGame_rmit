...
public class Blast_Demo4 : MonoBehaviour
{
    ...
    public void Init(PlayerAgent_Demo4 Agent)
    {
        playerAgent = Agent;
    }
    ...
    void OnTriggerEnter(Collider collider)
    {
        ...
        {
            Debug.Log("Shooted at Wall, AddReward -0.2 ");
            playerAgent.AddReward(-0.2f);
            Destroy(gameObject);
        }
        ...
        if (collider.gameObject.tag == "RaceB")
        {
            collider.gameObject.GetComponent<RaceB_Demo4>().Hited();
            Destroy(gameObject);
        }
